    ...
    <div class="card-modern p-3">
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
            <div class="nav nav-pills nav-pills-custom gap-2" id="notifFilterPills">
                ...
                <button class="nav-link" data-filter="warning">Sinh viên bị cảnh báo</button>
                <button class="nav-link" data-filter="general">Yêu cầu & Hệ thống</button>
            </div>
...
function renderPageNotifications() {
    ...
    if (activeFilter === 'warning') {
        filtered = allNotifications.filter(n => n.title.toLowerCase().includes('sinh viên bị cảnh báo') || n.title.toLowerCase().includes('cảnh báo') || n.message.toLowerCase().includes('cảnh báo'));
    ...
    listContainer.innerHTML = filtered.map(n => {
        const isWarning = n.title.toLowerCase().includes('cảnh báo') || n.message.toLowerCase().includes('cảnh báo') || n.title.toLowerCase().includes('sinh viên bị cảnh báo');
        const iconClass = isWarning ? 'warning' : 'info';
