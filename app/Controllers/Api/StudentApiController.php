...
class StudentApiController extends Controller {
    ...
    public function store() {
        ...
        if (empty($data['username']) || empty($data['last_name']) || empty($data['first_name']) || empty($data['email']) || empty($data['cohort']) || empty($data['major_id'])) {
            $this->jsonResponse(['status' => 'error', 'message' => 'Vui lòng nhập đầy đủ thông tin bắt buộc, bao gồm cả Khóa và Ngành học'], 400);
        }
    ...
    public function getMajors() {
        $db = Database::getInstance()->getConnection();
        $majors = $db->query("SELECT id, name FROM majors ORDER BY name ASC")->fetchAll();
        $this->jsonResponse(['status' => 'success', 'data' => $majors]);
    }
}
